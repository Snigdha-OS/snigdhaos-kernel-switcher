...
class FlowBox(Gtk.FlowBox):
    def __init__(self,kernel,active_kernel,manager_gui,source):
    ...
    def flowbox_community(self):
        ...
        if len(self.kernels) > 0:
            ...
            for cache in self.kernels:
                ...
                fb_child.set_name("%s %s %s" % (cache.name, cache.version, cache.repository))
                vbox_kernel_widgets = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=0)
                ...
                hbox_kernel_switch = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=0)
                ...
                label_kernel_name.set_markup("<b>%s</b> %s <i>%s</i>"% (cache.name, cache.version, cache.repository))
                label_kernel_name.set_selectable(True)
                vbox_kernel_widgets.append(label_kernel_name)
                tux_icon = Gtk.Picture.new_for_file(file=Gio.File.new_for_path(os.path.join(base_dir, "images/48x48/sks-tux.png")))
                tux_icon.set_can_shrink(True)
                for installed_kernel in self.manager_gui.installed_kernels:
                    if "{}-{}".format(installed_kernel.name, installed_kernel.version) == "{}-{}".format(cache.name, cache.version):
                        installed = True
                    if cache.name == installed_kernel.name:
                        if (cache.version > installed_kernel.version or cache.version != installed_kernel.version):
                            fn.logger.info("Kernel upgrade available - %s %s"% (cache.name, cache.version))
                            tux_icon = Gtk.Picture.new_for_file(file=Gio.File.new_for_path(os.path.join(base_dir, "images/48x48/sks-update.png")))
                            tux_icon.set_can_shrink(True)
                            label_kernel_name.set_markup("<b>*%s</b> %s <i>%s</i>"% (cache.name, cache.version, cache.repository))
    ...
    def flowbox_official(self):
        ...
        if len(self.kernels) > 0:
            ...
            for cache in sorted(self.kernels):
                ...
                if cache == latest:
                    tux_icon = Gtk.Picture.new_for_file(file=Gio.File.new_for_path(os.path.join(base_dir, "images/48x48/sks-new.png")))
                else:
                    tux_icon = Gtk.Picture.new_for_file(file=Gio.File.new_for_path(os.path.join(base_dir, "images/48x48/sks-tux.png")))
                tux_icon.set_content_fit(content_fit=Gtk.ContentFit.SCALE_DOWN)
                tux_icon.set_halign(Gtk.Align.START)
                ...
                for installed_kernel in self.manager_gui.installed_kernels:
                    if ("{}-{}".format(installed_kernel.name, installed_kernel.version) == cache.version):
                        installed = True
                if installed is True:
                    switch_kernel.set_state(True)
                    switch_kernel.set_active(True)
    ...
    def kernel_toggle_state(self, switch, data, kernel):
        fn.logger.debug("Switch toggled, kernel selected = %s %s" % (kernel.name, kernel.version))
        message = None
        title = None
        downgrade = False
        ...
        if fn.check_pacman_lockfile() is False:
            ...
            if switch.get_state() is False:  # and switch.get_active() is True:
                for inst_kernel in fn.get_installed_kernels():
                    if inst_kernel.name == kernel.name:
                        if self.source == "official":
                            if (inst_kernel.version > kernel.version.split("%s-" % inst_kernel.name)[1]):
                                downgrade = True
                ...
                if self.source == "community":
                    ...
                    if fn.check_pacman_repo(kernel.repository) is True:
                        ...
                        community_repo = False
                        fn.logger.error("%s pacman repo is not configured" % kernel.repository)
                elif self.source == "official":
                    message = "Confirm the install of <b>%s-%s</b>" % (kernel.name,kernel.version)
                if community_repo is False and self.source == "community":
                    mw = MessageWindow(title="Cannot find %s pacman repo" % kernel.repository,message="Enable the pacman repository then retry the installation",transient_for=self.manager_gui,detailed_message=False)
                    mw.present()
                else:
                    message_window = FlowBoxMessageWindow(title=title,message=message,action="install",kernel=kernel,transient_for=self.manager_gui,textview=self.manager_gui.textview,textbuffer=self.manager_gui.textbuffer,switch=switch,source=self.source,manager_gui=self.manager_gui,downgrade=downgrade)
            ...
            if switch.get_state() is True:
                ...
                if len(installed_kernels) > 1:
                    if self.source == "community":
                        message = "Confirm the removal of <b>%s-%s</b>" % (kernel.name,kernel.version)
                    elif self.source == "official":
                        message = "Confirm the removal of <b>%s-%s</b>" % (kernel.name,kernel.version)
                    message_window = FlowBoxMessageWindow(title="Kernel uninstallation",message=message,action="uninstall",kernel=kernel,transient_for=self.manager_gui,textview=self.manager_gui.textview,textbuffer=self.manager_gui.textbuffer,switch=switch,source=self.source,manager_gui=self.manager_gui,downgrade=downgrade)
                    message_window.present()
                    return True
                else:
                    switch.set_state(True)
                    # switch.set_active(False)
                    fn.logger.warning("You only have 1 kernel installed, and %s-%s is currently running, uninstall aborted."% (kernel.name, kernel.version))
            ...
            fn.logger.error("Pacman lockfile found, is another pacman process running ?")
            msg_win = MessageWindow(title="Warning",message="Pacman lockfile found, which indicates another pacman process is running",transient_for=self.manager_gui,detailed_message=False)
            msg_win.present()
            return True
...
class FlowBoxInstalled(Gtk.FlowBox):
    def __init__(self, installed_kernels, manager_gui, **kwargs):
        ...
        for installed_kernel in installed_kernels:
            tux_icon = Gtk.Picture.new_for_file(file=Gio.File.new_for_path(os.path.join(base_dir, "images/48x48/sks-tux.png")))
            fb_child = Gtk.FlowBoxChild()
            fb_child.set_name("%s %s" % (installed_kernel.name, installed_kernel.version))
            tux_icon.set_content_fit(content_fit=Gtk.ContentFit.SCALE_DOWN)
            tux_icon.set_halign(Gtk.Align.START)
            label_installed_kernel_version = Gtk.Label(xalign=0, yalign=0)
            label_installed_kernel_version.set_name("label_kernel_version")
            label_installed_kernel_version.set_markup("<b>%s</b> %s" % (installed_kernel.name, installed_kernel.version))
            label_installed_kernel_version.set_selectable(True)
            hbox_installed_version = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=0)
            ...
            vbox_uninstall_button = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=0)
            ...
            btn_uninstall_kernel.connect("clicked", self.button_uninstall_kernel, installed_kernel)
            vbox_kernel_widgets = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=0)
    ...
    def button_uninstall_kernel(self, button, installed_kernel):
        installed_kernels = fn.get_installed_kernels()
        if len(installed_kernels) > 1:
            fn.logger.info("Selected kernel to remove = %s %s" % (installed_kernel.name, installed_kernel.version))
            ...
            fn.logger.warning("You only have 1 kernel installed %s %s, uninstall aborted."% (installed_kernel.name, installed_kernel.version))
            msg_win = MessageWindow(
                title="Warning: Uninstall aborted",
                message=f"You only have 1 kernel installed\n"
...
class FlowBoxMessageWindow(Gtk.Window):
    def __init__(self,title,message,action,kernel,textview,textbuffer,switch,source,manager_gui,downgrade,**kwargs):
        super().__init__(**kwargs)
        self.set_title(title=title)
        self.set_modal(modal=True)
        self.set_resizable(False)
        self.set_icon_name("snigdhaos-kernel-switcher-tux")
        ...
        vbox_flowbox_message = Gtk.Box.new(orientation=Gtk.Orientation.VERTICAL, spacing=10)
    ...
    def on_button_yes_clicked(self, button):
        ...
        if fn.check_pacman_lockfile() is False:
            if self.action == "uninstall":
                progress_window = ProgressWindow(title="Removing kernel",action="uninstall",textview=self.textview,textbuffer=self.textbuffer,kernel=self.kernel,switch=self.switch,source=self.source,manager_gui=self.manager_gui,transient_for=self.manager_gui)
            if self.action == "install":
                progress_window = ProgressWindow(title="Installing kernel",action="install",textview=self.textview,textbuffer=self.textbuffer,kernel=self.kernel,switch=self.switch,source=self.source,manager_gui=self.manager_gui,transient_for=self.manager_gui)
        else:
            fn.logger.error("Pacman lockfile found, is another pacman process running ?")
